...
export function calculateWeeklyStats(draftedTeam: DraftedTeamWithPlayers, selectedGameweek: number): CalculatedWeeklyStats {
...
export function useWeeklyStats(draftedTeam: Ref<DraftedTeamWithPlayers>, selectedGameweek: Ref<number>) {
  const calculatedWeeklyStats = computed(() => {
    return calculateWeeklyStats(draftedTeam.value, selectedGameweek.value);
  });
