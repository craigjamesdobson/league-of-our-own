...
export const getters = {
  ...
  getSortedTeams:
    (state, getters, rootState, rootGetters) => (activeGameweek) => {
      // Create an empty array that will have additional properties added.
      const teams: any = []

      // Map all the incomplete weeks
      const incompleteWeeks = rootGetters[
        'fixture-data/getIncompleteWeeks'
      ].map((x) => x.week)

      state.draftedTeamData.forEach((team) => {
        // Loop through gameweek stats and return any that are before
        // or equal to the active gameweek and arent an incomplete week
        const filteredGameweeks = team.gameWeekStats.filter(
          (x) =>
            x.gameweek <= activeGameweek.value &&
            !incompleteWeeks.includes(x.gameweek.toString())
        )
