...
CREATE OR REPLACE FUNCTION "public"."get_drafted_teams_by_season"("active_season_param" "text") RETURNS "json"
    LANGUAGE "plpgsql"
    AS $$DECLARE
    result JSON;
BEGIN
...
ALTER FUNCTION "public"."get_drafted_teams_by_season"("active_season_param" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_drafted_teams_with_player_points_by_gameweek"("game_week_param" integer) RETURNS TABLE("drafted_team_id" integer, "team_name" "text", "team_email" "text", "team_owner" "text", "allowed_transfers" boolean, "weekly_stats" "json", "players" "json")
    LANGUAGE "plpgsql"
    AS $$BEGIN
      RETURN QUERY
      WITH filtered_player_statistics AS (
          SELECT
              ...
              SUM(CASE WHEN ps.red_card THEN 1 ELSE 0
   END) AS week_redcards,
  ...
  ELSE 0 END) AS week_cleansheets
   ...
   AND f.game_week = game_week_param
          GROUP BY
              ps.player_id
      )
      SELECT
  ...
  dt.drafted_team_id = dp.drafted_team
   ...
   pv.player_id
          LEFT JOIN filtered_player_statistics fps ON
   pv.player_id = fps.player_id
      WHERE
          dt.active_season = '24-25'
      GROUP BY
          dt.drafted_team_id,
          dt.team_name,
          dt.team_email,
...
CREATE OR REPLACE FUNCTION "public"."get_drafted_teams_with_player_points_by_gameweek_2"("game_week_param" integer) RETURNS TABLE("drafted_team_id" integer, "team_name" "text", "team_email" "text", "team_owner" "text", "allowed_transfers" boolean, "weekly_stats" "json", "players" "json")
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    RETURN QUERY
...
ALTER FUNCTION "public"."get_drafted_teams_with_player_points_by_gameweek_2"("game_week_param" integer) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_weekly_stats_for_gameweek"("target_week" integer) RETURNS TABLE("drafted_team_id" integer, "team_name" "text", "team_owner" "text", "goals" integer, "assists" integer, "clean_sheets" integer, "red_cards" integer, "total_points" integer, "week_points" integer, "weekly_winner" boolean, "prev_week_position" bigint)
  ...
  WITH max_points_cte AS (
    ...
    FROM (
      ...
      JOIN drafted_teams dt ON t.team = dt.drafted_team_id
      WHERE t.week = target_week
        AND dt.active_season = '24-25'
      GROUP BY t.team
    ) AS sub_query
  ), team_points_cte AS (
    SELECT 
...
CREATE OR REPLACE VIEW "public"."players_view" WITH ("security_invoker"='on') AS
 SELECT "p"."player_id",
    "p"."code",
...
ALTER TABLE ONLY "public"."drafted_teams"
    ADD CONSTRAINT "drafted_teams_drafted_team_id_key" UNIQUE ("drafted_team_id");



ALTER TABLE ONLY "public"."drafted_teams"
    ADD CONSTRAINT "drafted_teams_key_key" UNIQUE ("key");
...
ALTER TABLE ONLY "public"."player_statistics"
    ADD CONSTRAINT "player_statistics_id_key" UNIQUE ("id");
...
CREATE OR REPLACE TRIGGER "handle_updated_at" BEFORE UPDATE ON "public"."drafted_teams" FOR EACH ROW EXECUTE FUNCTION "extensions"."moddatetime"('updated_at');
...
CREATE POLICY "All access for authenticated" ON "public"."fixtures" TO "authenticated" USING (true);



CREATE POLICY "All access for authenticated" ON "public"."player_statistics" TO "authenticated" USING (true);



CREATE POLICY "All access for authenticated" ON "public"."weekly_statistics" USING (true);
...
CREATE POLICY "Enable read access for all users" ON "public"."fixtures" FOR SELECT USING (true);



CREATE POLICY "Enable read access for all users" ON "public"."player_statistics" FOR SELECT USING (true);
...
CREATE POLICY "Enable read access for all users" ON "public"."weekly_statistics" FOR SELECT USING (true);
...
GRANT ALL ON FUNCTION "public"."get_drafted_teams_by_season"("active_season_param" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."get_drafted_teams_by_season"("active_season_param" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_drafted_teams_by_season"("active_season_param" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_drafted_teams_with_player_points_by_gameweek_2"("game_week_param" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_drafted_teams_with_player_points_by_gameweek_2"("game_week_param" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_drafted_teams_with_player_points_by_gameweek_2"("game_week_param" integer) TO "service_role";
